...
Create a [dev cluster](/docs/core-concepts/ephemeral-environments/dev-clusters) and see your local changes deployed instantly.

## Prerequisites

- [Install](https://kubernetes.io/docs/tasks/tools/) `kubectl`
- [Install](../installation) the Uffizzi CLI.
...
### Clone the quickstart repo
  ...
  - A [`web.yaml`](https://github.com/UffizziCloud/quickstart/blob/main/manifests/web.yaml) manifest that includes a `service`, `ingress` and `deployment`.
...
Make sure you are in the root of the project directory, then run:
  ...
  - Start a [skaffold](https://skaffold.dev/docs/) process to build and deploy your project changes.  Alternatively, you can start a dev cluster in quiet mode (no output) 
...
### Open the application
  ...
  This command will open the application `ingress` in your default browser. You can read more about Uffizzi-provided ingress [here](../development/ingress).
...
important data stored in the cluster. However, [this is not recommended](/docs/handbook/dos-and-donts). 

</Steps>

Explore the [Usage section](cli/usage.mdx) to learn more about how to use the Uffizzi CLI, or checkout the [CLI reference](cli.mdx) for more information on specific commands.
