...
# Install Uffizzi Pro (~ 5 minutes)

Install Uffizzi Pro to create [ephemeral environments](/core-concepts/ephemeral-environments) on your own infrastructure.  
...
- A node with at least 1 CPU core and 1GB of memory (2 CPU cores and 2GB of memory recommended)  
...
<Callout type="info">
    Uffizzi Pro requires that your cluster have public network access, therefore it will **not** install on a local 
...
To configure DNS, create a record for the Controller hostname (e.g. `*.controller.example.com`) pointing to the IP address or hostname output by the `uffizzi install` command (i.e. the IP or hostname of the Controller service on your host cluster). 
