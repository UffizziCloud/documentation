...
When adding data to an ephemeral environment you should seed it on start up (See [Database Seeding](/docs/handbook/managing-state/database-seeding)). 
...
If you need to manipulate stateful services, you should [mock the service](/docs/handbook/mock-services) or create a copy  
that is internal to your environment, such as with [containerized databases](/docs/handbook/managing-state/database-seeding), so 
...
Use Uffizzi [dev clusters](/docs/core-concepts/ephemeral-environments/dev-clusters) for continuous development and deployment and 
[virtual clusters](/docs/core-concepts/ephemeral-environments/virtual-clusters) to test pre-production releases or against [different versions of 
Kubernetes](/docs/cli/commands/cluster#cluster-create).

## CI Pipelines

**Do configure Uffizzi with your CI platform**  

In addition to creating ephemeral environments from the [Uffizzi CLI](/docs/cli), you can also add Uffizzi to your 
...
tests and avoid the flakiness and resource contraints of running CI tests in pipeline runners. See our [CI guides](/docs/ci) for 
