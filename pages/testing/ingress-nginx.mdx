---
title: Reducing E2E Test Exectution by 6x for Ingress-Nginx
description: By swapping replacing KinD with Uffizzi virtual clusters, we dropped the execution time of the Ingress-Nginx project's end-to-end tests from 55 minutes to 9 minutes.
---

import { Callout } from 'nextra/components'

# Ingress-Nginx: 6x Faster Tests with Virtual Clusters

Our team at Uffizzi recently collaborated with the open-source project, [ingress-nginx](https://github.com/kubernetes/ingress-nginx), to tackle a persistent challenge: lengthy execution times for end-to-end (e2e) tests. The project has been using [kind](https://kind.sigs.k8s.io) (Kubernetes in Docker) for these tests, running them on GitHub Action runners. However, due to the runners' resource constraints, test execution was slow, allowing only a limited number of tests in parallel. **We saw an opportunity for improvement by introducing [Uffizzi virtual clusters](../../core-concepts/virtual-clusters), which, unlike KinD, operate on multiple compute instances and behave like standard Kubernetes clusters, including greater scalability.** This change aimed to enhance load handling and reduce test times. In this post, we'll walk through how this switch to Uffizzi unfolded and led to a 6x reduction in e2e execution for the ingress-nginx project.
...
The first e2e tests on Uffizzi initially took roughly 55 minutes to complete, just like `kind` on GitHub Actions. But because we're no longer constrained by the GitHub runner's resources, we can execute more tests in parallel. 21 parallel tests did execute much faster, but some tests randomly failed due to errors from the virtual Kubernetes master, implemented by `k3s` with an embedded SQLite database.

While provisioning faster virtual disks did marginally raise the performance ceiling, my testing showed SQLite was the bottleneck blocking more parallelization. When it became saturated, the Kubernetes API returned errors like "database is locked". Fortunately, external database options are available.
