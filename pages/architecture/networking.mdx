...
Each virtual cluster or dev cluster environment created by Uffizzi are restricted from communicating with other environments via Kubernetes [NetworkPolcies](https://kubernetes.io/docs/concepts/services-networking/network-policies/).

### Ingress & Load Balancing

Uffizzi supports two options for ingress: **`uffizzi`** (default) and **custom**. You can read more about these in detail [here](/ingress).  

If you choose the `uffizzi` IngressClass, then incoming coming traffic is routed through Uffizzi-managed load balancers. This allows Uffizzi to dynamically provision external IPs, domains, and TLS certificates to provide automatatically generate URLs for your ephemeral environments.  

Alternatively, you can configure your own custom IngressClass (or just a ClusterIP and LoadBalancer if you prefer). With this option, incoming traffic is routed through your LoadBalancers. 
